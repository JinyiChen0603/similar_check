...
**说明**：所有比较/查重逻辑都只在 `duplicate_logic.py` 中；`check_duplicate.py`、`similarity_report.py`、`input_and_compare.py` 仅是不同入口（数据来源与展示方式不同），底层均调用同一批函数。
...
| 手动输入一题（题干/解析/答案）与题库比较 | `input_and_compare.py` | `duplicate_logic.py` |
...
**可移植函数**：`compute_similarity`（17–24 行）、`check_duplicate` 的算法部分（109–141 行，但入参中的 `conn` 改为你提供的「已用题目列表」或封装成你后端的接口）。
...
- 输入：一道题（题干、解析、答案） + 题目列表（如 `questions.json`）。
- 行为：与该列表中每题用题目/解析/答案三维比较，得到综合相似度；只保留综合相似度 ≥ 阈值的题目，按综合相似度从高到低排序，可选只取前若干条。
- 入口：`input_and_compare.py`（终端手动输入题干/解析/答案后与 JSON 比较）；底层逻辑在 `duplicate_logic.py` 的封装函数中。
...
- **封装函数**（第 70–105 行）  
  - `find_similar_questions(user_question, question_list, *, threshold=0.3, top_k=None, weight_content=0.5, weight_explanation=0.3, weight_answer=0.2)`：  
    - 遍历 `question_list`，对每题调用 `compute_question_similarity(user_question, q, ...)`。  
    - **判断**：`if sim["overall"] >= threshold` 则加入结果。  
    - 按 `overall` 降序排序；若传入 `top_k` 则只保留前 `top_k` 条。  
    - 返回 `[(题目序号从1起, 题目dict, 相似度dict), ...]`。

### 3.3 移植时采用的方法

| 场景 | 采用方法 | 说明 |
|------|----------|------|
| **“一道题 vs 题库找相似题”** | `find_similar_questions(user_question, question_list, threshold=..., top_k=...)` | 直接调用即可，无需自己写循环与排序；题目列表从你网站 API/DB 获取。 |

**可移植函数**：`find_similar_questions`（70–105 行），依赖 `compute_question_similarity`（27–65 行）与 `compute_similarity`（17–24 行）。
...
| **“一道题 vs 题库”找相似题** | `find_similar_questions(user_question, question_list, ...)` | 见第三节；优先用此封装，无需自己写循环。 |
| **“相似题推荐/报告”的筛选逻辑** | `sim["overall"] >= threshold` | 先算 `compute_question_similarity`，再按综合相似度是否 ≥ 阈值决定是否展示。 |
| **题目数据来源** | 替换 `load_questions()` | 原实现读本地 `questions.json`；移植时改为从你的 API/DB 取题目列表，每题为 `{ "content", "explanation", "answer" }`。 |

**可移植函数**：`compute_similarity`（17–24 行）、`compute_question_similarity`（27–65 行）、`find_similar_questions`（70–105 行）。报告展示与数据加载按你网站的前端/后端方式重写即可。
...
移植重点在**数据结构**：保证题目对象包含 `content`、`explanation`、`answer`（或你映射后的键名），并在调用 `compute_question_similarity` / `find_similar_questions` 时使用同一键名。
...
| 一道题 vs 题目列表找相似题 | `duplicate_logic.py` | 70–105 | `find_similar_questions(user_question, question_list, threshold=..., top_k=...)` | 上者；题目列表由你方提供 |
| 题干查重（与已用题列表比较） | `duplicate_logic.py` | 109–141 | `check_duplicate` 内部：遍历 → `compute_similarity` → `sim >= threshold` → 排序截断 | 已用题目列表 `[(id, content), ...]` 由你方提供 |

**不需要移植**：`get_used_questions`、`get_db_connection`、`DB_PATH`（依赖本地 SQLite）；各脚本的 `load_questions`、本地 JSON 路径、`print` 等（改为你网站的数据源与展示方式）。
...
按上述方法与函数移植，即可在已有网站复现题干查重、一道题与题库比较、以及题目/解析/答案相似度比较功能。
